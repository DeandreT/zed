...
To make it easier to tell agents apart in the UI, give each one a custom name with the `display_name` setting, or use the rename button next to the agent in the **External Agents** page of [Agent Settings](./agent-settings.md):
...
```json [settings]
{
  "agent_servers": {
    "claude-enterprise": {
      "type": "registry",
      "registry_id": "claude-acp",
      "display_name": "Claude Code (Enterprise)"
    },
    "claude-max": {
      "type": "registry",
      "registry_id": "claude-acp",
...
`display_name` works for any external agent, whether it comes from the registry or a custom command.

### Using Different Accounts

Multiple instances of the same agent share that agent's own credential storage (for Claude Code, `~/.claude`), so by default they run on the same account. To put an instance on a different account, give it a secret environment variable: click the lock icon next to the agent in the **External Agents** page of [Agent Settings](./agent-settings.md) and add, for example, `CLAUDE_CODE_OAUTH_TOKEN` with a token generated by `claude setup-token`. Everything else (settings, commands, MCP servers, history) stays shared.

Secret values are stored in your system keychain — never in `settings.json` — and are injected into the agent's environment when it starts. The plain `env` setting remains available for non-secret variables; if the same variable name is set in both, the `env` value takes precedence.
